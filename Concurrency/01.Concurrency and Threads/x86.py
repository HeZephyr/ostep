...
class cpu:
    ...
    def __init__(self, memory, memtrace, regtrace, cctrace, compute, verbose):
        ...
        # conditions
        self.COND_GT        = 0     # greater than
        self.COND_GTE       = 1     # greater than or equal
        self.COND_LT        = 2     # less than
        self.COND_LTE       = 3     # less than or equal
        self.COND_EQ        = 4     # equal
        self.COND_NEQ       = 5     

        # registers in system
        self.REG_ZERO       = 0     # hidden zero-valued register
        # four general purpose registers
        self.REG_AX         = 1     
        self.REG_BX         = 2     
        self.REG_CX         = 3     
        self.REG_DX         = 4     
        self.REG_SP         = 5     # stack pointer
        self.REG_BP         = 6     # base pointer
    ...
    # print out memory but only if it's been touched
    ...
    # get the list of registers and conditions
    ...
    # move src to the dst register    
    def move_i_to_r(self, src, dst):
        self.registers[dst] = src
        return 0

    # memory: value, register, register; move the src to memory
    def move_i_to_m(self, src, value, reg1, reg2):
        tmp = value + self.registers[reg1] + self.registers[reg2]
        self.memory[tmp] = src
        return 0

    # memory: value, register, register; move the value of memory to the dst register
    ...
    # memory: value, register, register; move the src register to memory
    def move_r_to_m(self, src, value, reg1, reg2):
        tmp = value + self.registers[reg1] + self.registers[reg2]
        self.memory[tmp] = self.registers[src]
        return 0

    # move the src register to the dst register
    def move_r_to_r(self, src, dst):
        self.registers[dst] = self.registers[src]
        return 0

    # add the src immediate to the dst register
    def add_i_to_r(self, src, dst):
        self.registers[dst] += src
        return 0

    # add the src register to the dst register
    def add_r_to_r(self, src, dst):
        self.registers[dst] += self.registers[src]
        return 0

    # sub the src immediate from the dst register
    def sub_i_to_r(self, src, dst):
        self.registers[dst] -= src
        return 0

    # sub the src register from the dst register
    ...
    # atomic exchange: src register with memory value
    # memory: value + reg1 + reg2
    ...
    # fetchadd: src register with memory value
    ...
    # call a subroutine
    def call(self, targ):
        # Decrement the stack pointer (SP) to allocate space for the return address
    ...
    def ret(self):
        ...
        # Increment the stack pointer (SP) to deallocate the space used for the return address
        self.registers[self.REG_SP] += 4
    ...
    # push the value of a memory location onto the stack
    ...
    # pop the value off the stack into a register
    def pop_r(self, dst):
    ...
    def getarg(self, arg):
        ...
        if tmp[0] == '$':   # immediate, extract the number after the $
        ...
        elif tmp[0] == '%': # register, extract the register name after the %
            register = tmp.split('%')[1]
            return self.register_translate(register), 'TYPE_REGISTER'
        elif tmp[0] == '(': # memory, extract the register name after the %
        ...
        elif tmp[0] == '.': # label, extract the label name
            targ = tmp
            return targ, 'TYPE_LABEL'
        elif tmp[0].isalpha() and not tmp[0].isdigit(): # variable, extract the variable name
            ...
            if len(s) == 1:
                ...
                value = neg * int(s[0])
                t = s[1].split(')')[0].split(',') # get the registers
                if len(t) == 1:
    ...
    def load(self, infile, loadaddr):
        ...
        # first pass: assign labels and variables
        for line in fd:
            cline = line.rstrip()
            # print 'PASS 1', cline
            ...
            elif tmp[0][0] == '.':  # associate label with pc
                assert(len(tmp) == 1)
        ...
        for line in fd:
            ...
            if cline[0] != '.':
                ...
                if opcode == 'mov':
                    ...
                    # can't move memory to memory, immediate to immediate
