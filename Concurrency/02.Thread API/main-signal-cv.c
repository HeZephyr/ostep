...
typedef struct __synchronizer_t {
    pthread_mutex_t lock;   // mutex ensuring exclusive access to "done"
    pthread_cond_t cond;    // conditional variable indicating "done" value
    int done;               // value protected by lock, 0 if not done, 1 if done
} synchronizer_t;
...
void signal_done(synchronizer_t *s) {
    ...
    s->done = 1;
    Pthread_cond_signal(&s->cond);  // signal to waiting thread
...
void signal_wait(synchronizer_t *s) {
    Pthread_mutex_lock(&s->lock);
    while (s->done == 0)
	    Pthread_cond_wait(&s->cond, &s->lock);  // wait until signalled
    Pthread_mutex_unlock(&s->lock);
}
