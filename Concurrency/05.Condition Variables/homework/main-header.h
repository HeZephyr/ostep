...
// Variables to control tracing and timing
int do_trace = 0;
int do_timing = 0;

// Macros for defining pause points in the code
...
// Arrays to store pause times for producers and consumers
int producer_pause_times[MAX_THREADS][7];
int consumer_pause_times[MAX_THREADS][7];

// Mutex to avoid interleaving of print out from threads
...
// Function to print pointers in the buffer for tracing
...
// Function to print the buffer for tracing
...
// if we're tracing, print out the buffer
void do_eos() {
    if (do_trace) {
...
// Function to add pauses in the code execution for tracing purposes
...
// Function to ensure that an expression holds true; otherwise, print an error message and exit
...
/**
 * Parses the pause string to initialize the pause_array for producers or consumers.
 * 
 * @param str              The pause string to be parsed.
 * @param name             The name of the producers or consumers (for error messages).
 * @param expected_pieces  The expected number of pieces separated by colons.
 * @param pause_array      The array to store the parsed pause amounts.
 */
void parse_pause_string(char *str, char *name, int expected_pieces, 
    ...
    int index = 0;						// Index for accessing the pause_array

    char *copy_entire = strdup(str);	// Make a copy of the input string
    char *outer_marker;					// Pointer for strtok_r to mark the outer delimiter
    int colon_count = 0;				// Counter for the number of colons encountered
	// Tokenize the string using colons as delimiters
    ...
    while (p) {
		...
		char *inner_marker;									// Pointer for strtok_r to mark the inner delimiter
		char *copy_piece = strdup(p);						// Make a copy of the piece
		char *c = strtok_r(copy_piece, ",", &inner_marker);	// Tokenize using commas
		int comma_count = 0;								// Index for accessing the pause amounts

		int inner_index = 0;								// Index for accessing the pause amounts
		while (c) {
