...
# Program Explanation
1. 原子性违规
   * `atomicity.c`：展示了不谨慎的校验使用是如何导致代码崩溃的
   * `atomicity_fixed.c`：展示如何用锁解决问题
2. 顺序违规
   * `ordering.c`：排序问题
   * `ordering_fixed.c`：展示如何使用条件变量解决问题
3. 死锁
   * `deadlock.c`：显示简单的两周期死锁
   * `deadlock_run.sh`：多次运行上述程序的脚本，直到遇到死锁并确信死锁可能发生为止
