...
STATE_SEEK   = 1    # seeking to the right track
STATE_ROTATE = 2    # rotating to the right block
STATE_XFER   = 3    # transferring data
STATE_DONE   = 4    # done with request
...
class Disk:
    def __init__(self, addr, addrDesc, lateAddr, lateAddrDesc,
                 ...
                 graphics, zoning):
        self.addr              = addr               # list of request addresses
        self.addrDesc          = addrDesc           # descriptor for generating requests
        self.lateAddr          = lateAddr           # list of late request addresses
        self.lateAddrDesc      = lateAddrDesc       # descriptor for generating late requests
        self.policy            = policy             # scheduling policy
        self.seekSpeed         = seekSpeed          # speed of seek
        self.rotateSpeed       = rotateSpeed        # speed of rotation
        self.skew              = skew               # skew of blocks on disk
        self.window            = window             # scheduling window
        self.compute           = compute            # compute the answers
        self.graphics          = graphics           # graphical mode
        self.zoning            = zoning             # zoning of blocks on disk
    ...
    def InitBlockLayout(self):
        # 初始化块信息列表、块到磁道映射、块到角度映射、磁道开始结束、块角度偏移
        self.blockInfoList    = []
        ...
        # 初始化外部磁道
        track        = 0 # outer track
        angleOffset  = 2 * self.blockAngleOffset[track] # 角度偏移量
