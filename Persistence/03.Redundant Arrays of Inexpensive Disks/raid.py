...
class disk:
    def __init__(self, seekTime=10, xferTime=0.1, queueLen=8):
        ...
        # disk geometry
        self.numTracks      = 100                                   # number of tracks
        self.blocksPerTrack = 100                                   # number of blocks per track
        self.blocksPerDisk  = self.numTracks * self.blocksPerTrack  # number of blocks per disk

        # stats
        self.countIO   = 0  # number of I/Os
        self.countSeq  = 0  # number of sequential I/Os
        self.countNseq = 0  # number of nearly sequential I/Os
        self.countRand = 0  # number of random I/Os
        self.utilTime  = 0  # total time disk is busy
    ...
    def enqueue(self, addr):
        """
        Adds a read/write operation to the disk's queue.

        Args:
            addr (int): Address of the block for the operation.
        """
        assert(addr < self.blocksPerDisk)   # sanity check
        self.countIO += 1
        ...
        if currTrack == newTrack or diff < self.blocksPerTrack:
            if diff == 1:
                ...
                self.countNseq += 1 # nearly sequential
            # add the diff * read block time
            self.utilTime += (diff * self.xferTime)
        else:
            self.countRand += 1
            # add the seek time + read block time
...
class raid:
    def __init__(self, chunkSize='4k', numDisks=4, level=0, timing=False, reverse=False, solve=False, raid5type='LS'):
        ...
        self.chunkSize = int(chunkSize / BLOCKSIZE) # chunk size in blocks
        self.numDisks  = numDisks       # number of disks in the RAID
        self.raidLevel = level          # RAID level (0, 1, 4, or 5)
        self.timing    = timing         # timing mode (True) or mapping mode (False)
        self.reverse   = reverse        # Flag for reverse mode
        self.solve     = solve          # Flag for solving mode
        self.raid5type = raid5type      # RAID 5 type (left-asymmetric or left-symmetric)

        # assert that the chunk size is a multiple of the block size
        if (chunkSize % BLOCKSIZE) != 0:
            print('chunksize (%d) must be multiple of blocksize (%d): %d' % (chunkSize, BLOCKSIZE, self.chunkSize % BLOCKSIZE))
            exit(1)
        
        # assert that the RAID-1 disks are a multiple of two
        if self.raidLevel == 1 and numDisks % 2 != 0:
            print('raid1: disks (%d) must be a multiple of two' % numDisks)
            exit(1)

        if self.raidLevel == 4:
            self.blocksInStripe = (self.numDisks - 1) * self.chunkSize  # Number of blocks in a stripe for RAID 4
            self.pdisk = self.numDisks - 1      # Parity disk index for RAID 4
        if self.raidLevel == 5:
            self.blocksInStripe = (self.numDisks - 1) * self.chunkSize  # Number of blocks in a stripe for RAID 5
            self.pdisk = -1                    # Parity disk index for RAID 5

        # Create disk objects for the RAID array
        self.disks = []
    ...
    def stats(self, totalTime):
        for d in range(self.numDisks):
            ...
            # disk utilization
            if totalTime > 0.0:
                util = (100.0*float(s[4])/totalTime)    # s[4] is the disk d utilization time
            else:
                util = 0.0
            if s[4] == totalTime:
