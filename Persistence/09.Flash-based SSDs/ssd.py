...
class ssd:
    def __init__(self, ssd_type, num_logical_pages, num_blocks, pages_per_block,
        ...
        # SSD type
        self.TYPE_DIRECT = 1    # direct mapping
        self.TYPE_LOGGING = 2   # logging
        self.TYPE_IDEAL = 3     # ideal
        ...
        # size
        self.num_logical_pages = num_logical_pages  # logical pages
        self.num_blocks = num_blocks                # physical blocks
        self.pages_per_block = pages_per_block      # pages per block
        ...
        # 初始化每个页面的状态为无效
        self.state = {i: self.STATE_INVALID for i in range(self.num_pages)}

        # 初始化每个页面的数据为空字符串
        self.data = {i: ' ' for i in range(self.num_pages)}
        ...
        # gc counts
        self.gc_count = 0                           # number of GCs
        self.gc_current_block = 0                   # current block to start GC
        self.gc_high_water_mark = high_water_mark   # blocks used before GC
        self.gc_low_water_mark = low_water_mark     # blocks used before stopping GC
        
        # is whether to show the GC trace
        self.gc_trace = trace_gc
        self.show_state = show_state

        # 初始化每个块的使用情况为0
        self.gc_used_blocks = {i: 0 for i in range(self.num_blocks)}

        # counts so as to help the GC
        # 初始化每个块的 live count 为 0
        self.live_count = {i: 0 for i in range(self.num_blocks)}

        # FTL
        # 初始化 forward_map，reverse_map
        self.forward_map = {i: -1 for i in range(self.num_logical_pages)}
        self.reverse_map = {i: -1 for i in range(self.num_pages)}

        # stats
        self.physical_erase_count = {}  # 每个块的擦除次数
        self.physical_read_count = {}   # 每个块的读次数
        self.physical_write_count = {}  # 每个块的写次数
    ...
    def write_direct(self, page_address, data):
        ...
        # erase the block and program the new data
    ...
    #  Write data directly to a page address in ideal mode.
    ...
    # Check if a block is free for writing.
    ...
    # Get the cursor position for writing data.
    def get_cursor(self):
