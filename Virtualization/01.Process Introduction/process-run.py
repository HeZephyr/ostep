...
# Function to set random seed for Python 2 and Python 3 compatibility
...
class scheduler:
    ...
    # Method to load program description into a process
    def load(self, program_description):
        proc_id = self.new_process()
        # Split the program description into two parts: number of instructions and percentage chance of CPU instructions
        tmp = program_description.split(':')
        # Check if the program description is valid
        if len(tmp) != 2:
        ...
        # Extract the number of instructions and the percentage chance of CPU instructions from the program description
        num_instructions, chance_cpu = int(tmp[0]), float(tmp[1])/100.0
         # Iterate over the specified number of instructions
    ...
    # Method to move a process to ready state
    def move_to_ready(self, expected, pid=-1):
        if pid == -1:
            pid = self.curr_proc
        # Ensure that the current state of the specified process matches the expected state
        assert(self.proc_info[pid][PROC_STATE] == expected)
        self.proc_info[pid][PROC_STATE] = STATE_READY
        return

    # Method to move a process to wait state
    ...
    # Method to move a process to running state
    def move_to_running(self, expected):
        assert(self.proc_info[self.curr_proc][PROC_STATE] == expected)
        self.proc_info[self.curr_proc][PROC_STATE] = STATE_RUNNING
        return

    # Method to move a process to done state
    def move_to_done(self, expected):
        assert(self.proc_info[self.curr_proc][PROC_STATE] == expected)
        self.proc_info[self.curr_proc][PROC_STATE] = STATE_DONE
        return

    # Method to select the next process to run
    def next_proc(self, pid=-1):
        # If pid is not provided, the current process will be moved to running
        if pid != -1:
            self.curr_proc = pid
        ...
        for pid in range(self.curr_proc + 1, len(self.proc_info)):
            if self.proc_info[pid][PROC_STATE] == STATE_READY:
                ...
                self.move_to_running(STATE_READY)
                return
        # If no process is found in the READY state after the current process, loop back and check processes from the beginning
    ...
    # Method to get the number of processes
    def get_num_processes(self):
        return len(self.proc_info)

    # Method to get the number of instructions for a process
    def get_num_instructions(self, pid):
        return len(self.proc_info[pid][PROC_CODE])

    # Method to get an instruction for a process
    def get_instruction(self, pid, index):
        return self.proc_info[pid][PROC_CODE][index]

    # Method to get the number of active processes
    ...
    # Method to get the number of runnable processes
    ...
    # Method to get the number of IOs in flight
    ...
    # Method to check for process switch
    ...
    # Method to check if the current process is done
    ...
    def run(self):
        # Initialize the clock tick counter
        clock_tick = 0

        # If there are no processes to execute, return
        ...
        while self.get_num_active() > 0:
            ...
            # Check if any IO operation has finished
            io_done = False
            for pid in range(len(self.proc_info)):
...
if __name__ == "__main__":
    ...
    # Create an OptionParser object for parsing command-line arguments
    parser = OptionParser()

    # Add command-line options with their explanations
    ...
    # Ensure that the process switch behavior and IO done behavior are valid
    ...
    # Create a scheduler object with the specified process switch behavior, IO done behavior, and IO length
    s = scheduler(options.process_switch_behavior, options.io_done_behavior, options.io_length)

    # Load program instructions into the scheduler if a program is specified; otherwise, load process descriptions
    if options.program != '':
    ...
    # If the solve option is not enabled, print a trace of what would happen when running the processes
    ...
    # If print_stats option is enabled, print statistics
