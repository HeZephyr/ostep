...
# Length of time slices at each level
quantum = {}
# 
if options.quantumList != '':
    ...
    for i in range(numQueues):
        ...
        qc -= 1
else: # If not provided, all time slices are the same length at all levels
    for i in range(numQueues):
        quantum[i] = int(options.quantum)

# Stores time quotas for each queue
allotment = {}
...
# Set the index of the highest priority queue to the number of queues minus one
hiQueue = numQueues - 1
...
# Counter to keep track of the number of jobs
jobCnt = 0

# jlist 'startTime,runTime,ioFreq:startTime,runTime,ioFreq:...'
# If the user has provided a job list using the -j or --jlist option
if options.jlist != '':
    # Split the job list into individual job information separated by ':'
    allJobs = options.jlist.split(':')
    for j in allJobs:
        # Split each job information into start time, run time, and I/O frequency
        jobInfo = j.split(',')
        if len(jobInfo) != 3:
        ...
        # Extract start time, run time, and I/O frequency from the job information
        startTime = int(jobInfo[0])
        runTime   = int(jobInfo[1])
        ioFreq    = int(jobInfo[2])
        # Initialize job parameters and store them in the job dictionary
        job[jobCnt] = {'currPri':hiQueue, 'ticksLeft':quantum[hiQueue],
                       ...
                       'firstRun':-1}
        # If the start time of the job is not in ioDone, add it to ioDone to indicate job start
        if startTime not in ioDone:
            ioDone[startTime] = []
        # Record job start in ioDone
...
while finishedJobs < totalJobs:
    ...
    if options.boost > 0 and currTime != 0:
        # Check whether the boost cycle point has been reached
    ...
    if currTime in ioDone:
        for (j, type) in ioDone[currTime]:
            q = job[j]['currPri']
            # Set the 'doingIO' flag of job j to False, indicating it is no longer performing I/O
            job[j]['doingIO'] = False
            print('[ time %d ] %s by JOB %d' % (currTime, type, j))
            # Depending on whether the iobump option is enabled or if the completion type is 'JOB BEGINS',
            # reinsert job j into its current priority queue
    ...
    currQueue = FindQueue()
    # If no job is found in any queue (system idle)
    if currQueue == -1:
    ...
    # Decrement the remaining time and ticks for the current job
    job[currJob]['timeLeft']  -= 1
    job[currJob]['ticksLeft'] -= 1

    # If this is the first run of the job, record the current time
    ...
    # CHECK FOR IO
    issuedIO = False
    # If the job has an I/O frequency greater than zero and it's time for an I/O
    if ioFreq > 0 and (((runTime - timeLeft) % ioFreq) == 0):
        ...
        # Mark the job as currently performing I/O
        ...
        # Determine the time when the I/O will complete
