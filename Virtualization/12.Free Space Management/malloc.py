...
class malloc:
    ...
    # add the address and size to the map
    ...
    def malloc(self, size):
        ...
        for i in range(len(self.freelist)):
            # get the address and size of the free block
    ...
    def free(self, addr):
        ...
        # according to the policy, add the block to the free list
        ...
        if self.coalesce == True:
            ...
            for i in range(1, len(self.freelist)):
                ...
                # if the end of the current block is the same as the start of the next block
    ...
    # print the free list information
    def dump(self):
...
if __name__ == '__main__':
    ...
    # calculate the percent of operations that are allocations
    ...
    # randomly generate or use the list of operations
    if options.opsList == '':
        c = 0
        j = 0
        while j < int(options.opsNum):
            # Flag to determine whether an operation was performed
            pr = False
            if random.random() < percent:
                # Generate a random size for allocation
                size     = int(random.random() * int(options.opsRange)) + 1
                # Allocate memory
                ptr, cnt = m.malloc(size)
                if ptr != -1:
                    # Store the pointer address and operation index
                    p[c] = ptr
                    L.append(c)
