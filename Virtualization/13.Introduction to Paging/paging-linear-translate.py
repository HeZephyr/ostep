...
if __name__ == '__main__':
    ...
    # create the page table and mark all entries as not used
    used = array.array('i')
    pt   = array.array('i')
    ...
    for v in range(0,vpages):
        ...
        # Loop until a valid page is assigned to the virtual page number (v)
        while done == 0:
            # Generate a random number to determine if the page is used
            if ((random.random() * 100.0) > (100.0 - float(options.used))):
                ...
                if options.verbose == True:
                    ...
                    print('  ', end='')
                print('0x%08x' % 0)
                # Insert -1 into the page table at index v to indicate an invalid page
                pt.insert(v,-1)
                done = 1
    print(''            )
