...
class OS:
    def __init__(self):
        ...
        # 11111 11111 00000 -> 1111 1111 1100 0000
    ...
    # initialize a page of memory to 0x7f
    ...
    # given a virtual address, return the page table entry
    ...
    def getPageDirEntry(self, pid, virtualAddr, printStuff):
        # get the page directory entry of the current process
        pageDir = self.pdbr[pid]
        pdeBits = (virtualAddr & self.PDE_MASK) >> self.PDE_SHIFT
        pdeAddr = (pageDir << self.pageBits) | pdeBits
        # get the contents of the PDE
        pde = self.memory[pdeAddr]
        # extract the valid bit and the page table page number
        valid = (pde & 0x80) >> 7
        ptPtr = (pde & 0x7f)
        if printStuff == True:
    ...
    def procAlloc(self, pid, numPages):
        ...
        # trace which virtual pages are allocated
        used = {}
        ...
        for vp in range(0, numPages):
            ...
            # allocate a page of memory, and fill it with random data
    ...
    # get the page directory base register for a process
    def getPDBR(self, pid):
        return self.pdbr[pid]

    # get the value of a physical address
