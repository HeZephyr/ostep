...
# according the index to judge whether the page is in the memory
def hfunc(index):
    if index == -1:
        return 'MISS'
    else:
...
if __name__ == '__main__':
    ...
    addrList = []
    # according to the addressfile or given addresser or randomly generate the address
    if addressFile != '':
        fd = open(addressFile)
        for line in fd:
    ...
    if options.solve == False:
        ...
        count = 0 # number of pages in memory
        memory = [] # the memory list
        hits = 0 # number of hits
        miss = 0 # number of misses

        # according to the policy to set the left and right string
        if policy == 'FIFO':
            leftStr = 'FirstIn'
            riteStr = 'Lastin '
        elif policy == 'LRU':
                ...
                if count == cachesize:
                    ...
                    if policy == 'FIFO' or policy == 'LRU':
                        ...
                        for pageIndex in range(0, count):
                            ...
                            for futureIdx in range(addrIndex + 1, len(addrList)):
                                futurePage = int(addrList[futureIdx])
                                # If the current page is referenced in the future, update the whenReferenced
                                if page == futurePage:
                                    whenReferenced = futureIdx
                                    break
